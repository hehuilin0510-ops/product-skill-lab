...
- 活动原型生成新增活动礼物命名锁定，未确认名称时使用一致的证据安全占位。
- 每张活动状态图保留统一头图，弹窗和下拉状态继承完整父页面。
- 奖励占位按礼物、头像框、徽章/勋章、金豆/金币、积分和通用奖励六类统一管理。
- 原型驱动 PRD 新增三列表格与一体画板双模式，起草前先确认交付形态。
- 一体画板支持原型、流程箭头和完整需求卡，并强化 Pencil 主文件、层级、碰撞与可读性检查。
- 下载包新增画板模式参考和同尺寸分享图优化脚本。
