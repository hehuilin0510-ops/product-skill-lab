...
6. 检查画布裁切、模块缺失、中文文案、头图与奖励资源一致性。
7. 输出单页图片、总览大画板和可交接目录。
...
8. 更新后回读目标内容，检查旧口径、旧截图、版本记录和图片数量。
...
- 活动原型生成公开包同步复核并保留礼物命名、统一头图、六类奖励占位和 QA 规则。
- 原型驱动 PRD 新增唯一当前交付入口和固定编号目录结构。
- 强化 Pencil 主文件与图片依赖的成套管理，避免 Missing dependencies。
- 新增重复主文件、散落依赖、旧导出和多份 latest 目录的交付阻断项。
- 新增交付后目录树、依赖解析、画板、导出图和 QA 图回读要求。
- 两个公开下载包均补充中文 README，说明用途、场景、能力、输入输出和使用示例。
